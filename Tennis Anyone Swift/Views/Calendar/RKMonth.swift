...
struct RKMonth: View {
    ...
    func dateTapped(date: Date) {
        if self.isEnabled(date: date) {
            ...
            if self.rkManager.blockedDatesContains(date: date) {
                if let ndx = self.rkManager.blockedDatesFindIndex(date: date) {
                    rkManager.blockedDates.remove(at: ndx)
                }
            } else {
                self.rkManager.blockedDates.append(date)
            }
    ...
    func isSpecialDate(date: Date) -> Bool {
        return
            isOneOfBlockedDates(date: date)
    }
    
    func isOneOfBlockedDates(date: Date) -> Bool {
        return self.rkManager.blockedDatesContains(date: date)
    }
